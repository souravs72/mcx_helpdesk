  ...
  <div v-if="ticket.data" class="flex flex-col">
    ...
    <div class="flex overflow-hidden h-full w-full">
      ...
      <section class="flex flex-col flex-1 w-full md:max-w-[calc(100%-382px)]">
        ...
        <!-- MCX: proactive SLA warning -->
        <div
          v-if="isSlaAtRisk && !isSlaBreach"
          class="px-6 md:px-10 mt-3"
        >
          <Alert
            :title="slaRiskMessage"
            theme="yellow"
            class="text-p-sm [&_.size-4]:relative [&>.size-4]:top-[3.5px] border border-amber-200"
          />
        </div>
        <!-- MCX: SLA breach alert -->
        <div
          v-if="isSlaBreach"
          class="px-6 md:px-10 mt-3"
        >
          <Alert
            title="Your ticket has exceeded its expected resolution time and is being given priority attention by our senior team."
            theme="red"
            class="text-p-sm [&_.size-4]:relative [&>.size-4]:top-[3.5px] border border-red-200"
          />
        </div>
...
import TicketCustomerTemplateFields from "@/pages/ticket/TicketCustomerTemplateFields.vue";
import TicketFeedback from "@/pages/ticket/TicketFeedback.vue";
...
onMounted(() => {
  ...
  const CLEANUP_KEY = "dismissBanner_lastCleanup";
...
const isSlaBreach = computed(
  () =>
    ticket.data?.agreement_status === "Failed" &&
    ticket.data?.status !== "Closed"
);

const isSlaAtRisk = computed(() => {
  const risk = ticket.data?.mcx_sla_risk_level;
  return risk && risk !== "None" && !isSlaBreach.value;
});

const slaRiskMessage = computed(() => {
  const risk = ticket.data?.mcx_sla_risk_level;
  const target = ticket.data?.mcx_sla_risk_target || "resolution";
  if (risk === "Critical") {
    return `Your ticket is approaching its ${target.toLowerCase()} deadline. Our team is prioritizing it to resolve before SLA breach.`;
  }
  return `Your ticket is being actively handled. We are working to meet the ${target.toLowerCase()} commitment on time.`;
});

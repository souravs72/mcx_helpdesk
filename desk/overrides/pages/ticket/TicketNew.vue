...
const template = createResource({
  ...
  onSuccess: async (data) => {
    ...
    await setupCustomizations(template, {
      doc: templateFields,
      call,
      router,
      $dialog,
...
function clearSubIssueLinkFilters() {
  const field = template.data?.fields?.find(
...
function applySubIssueFilters(issueType: string) {
  const field = template.data?.fields?.find(
    (f) => f.fieldname === "sub_issue_type"
  );
  if (!field) return;
  if (!issueType) {
    clearSubIssueLinkFilters();
    templateFields.sub_issue_type = "";
    return;
  }
  field.link_filters = JSON.stringify([["issue_type", "=", issueType]]);
